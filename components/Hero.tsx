...
export default function Hero() {
  return (
    <section className="relative w-full overflow-hidden bg-gradient-to-br from-[#0F172A] via-[#0057FF] to-[#0F172A]">
      {/* Video background */}
      <video
        autoPlay
        loop
      ...
      {/* Blue gradient overlay for legibility + tech feel */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#0F172A]/90 via-[#0057FF]/40 to-[#0F172A]/90" />
      ...
      {/* Blue glow */}
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-[#0057FF]/30 rounded-full blur-3xl translate-x-1/2 translate-y-1/2" />
      ...
      <div className="container mx-auto px-4 sm:px-6 py-12 sm:py-16 md:py-24 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-12 items-center">
          ...
          <div className="text-center lg:text-left">
            <div className="inline-flex items-center gap-2 bg-[#FFD400]/15 border border-[#FFD400]/30 rounded-full px-4 py-2 mb-6">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap justify-center lg:justify-start gap-3 mb-10">
              {['Conserto Rápido', 'Orçamento Gratuito', 'Garantia no Serviço', 'Entrega para todo Brasil'].map((item) => (
                <div key={item} className="flex items-center justify-center sm:justify-start gap-2 text-sm text-blue-100">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap justify-center lg:justify-start gap-4">
              <Link
                href="/contato"
                className="inline-flex items-center justify-center gap-2 bg-[#FFD400] text-[#0F172A] font-black py-4 px-8 rounded-xl hover:bg-[#0057FF] hover:text-white transition-all duration-200 hover:scale-105 hover:shadow-[0_0_30px_rgba(255,212,0,0.35)] text-base"
              >
                Solicitar Orçamento
              ...
              <Link
                ...
                className="inline-flex items-center justify-center gap-2 border-2 border-white/30 text-white font-bold py-4 px-8 rounded-xl hover:border-[#FFD400] hover:text-[#FFD400] transition-all duration-200 text-base"
              >
                Ver Produtos
              </Link>
            </div>
            ...
            <div className="relative z-10 w-full max-w-xs sm:max-w-sm">
              <div className="grid grid-cols-2 gap-2.5 sm:gap-3 mb-2.5 sm:mb-3">
                {services.map(({ icon: Icon, title, desc }, i) => (
                  <div
                    key={title}
                    className={`rounded-2xl p-4 sm:p-5 border backdrop-blur-sm hover:border-[#FFD400]/50 transition-all duration-300 hover:-translate-y-1 ${i % 2 === 0 ? 'bg-[#FFD400]/10 border-[#FFD400]/25' : 'bg-white/10 border-white/20'}`}
                  >
                    ...
                    <p className="text-blue-100/70 text-[11px] sm:text-xs">{desc}</p>
                  </div>
                ))}
              </div>

              {/* Stats bar */}
              <div className="bg-[#FFD400] rounded-2xl p-3 sm:p-4 flex items-center justify-around shadow-[0_0_30px_rgba(255,212,0,0.3)]">
                <div className="text-center">
                  <div className="font-black text-[#0F172A] text-xl sm:text-2xl leading-none">+5000</div>
                  <div className="text-[#0F172A]/60 text-[9px] sm:text-[10px] font-semibold mt-0.5">Óculos Recuperados</div>
                </div>
                <div className="w-px h-10 bg-black/15" />
                <div className="text-center">
                  <div className="font-black text-[#0F172A] text-xl sm:text-2xl leading-none">98%</div>
                  <div className="text-[#0F172A]/60 text-[9px] sm:text-[10px] font-semibold mt-0.5">Satisfação</div>
                </div>
                <div className="w-px h-10 bg-black/15" />
                <div className="text-center">
                  <div className="font-black text-[#0F172A] text-xl sm:text-2xl leading-none">24h</div>
                  <div className="text-[#0F172A]/60 text-[9px] sm:text-[10px] font-semibold mt-0.5">Prazo Médio</div>
                </div>
              </div>
            </div>
          </div>
