...
export default function Footer() {
  return (
    <footer className="bg-gradient-to-br from-[#0F172A] via-[#0F172A] to-[#0057FF] pt-16 pb-8 mt-auto">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-14">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-blue-100/60">
              {[
                ...
                <li key={href}>
                  <Link href={href} className="hover:text-[#7FB2FF] transition-colors hover:translate-x-1 inline-block duration-200">
                    {label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <p className="text-blue-100/60 text-sm mb-4 leading-relaxed">
            ...
            <form className="flex flex-col gap-3 mb-8">
              ...
              <button
                type="submit"
                className="w-full bg-[#FFD400] text-[#0F172A] font-black py-3 rounded-xl hover:bg-yellow-300 transition-all duration-200 hover:scale-[1.02] text-sm"
              >
                Cadastrar
              </button>
            </form>
            ...
            <div className="flex items-center gap-3">
              {[
                ...
                <a
                  key={i}
                  href={href}
                  className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:bg-[#FFD400] hover:text-[#0F172A] hover:border-[#FFD400] transition-all duration-200 hover:scale-110"
                >
        ...
        {/* Bottom bar */}
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-blue-100/50">
          <div className="flex items-center gap-4">
            <p>© {new Date().getFullYear()} Socorro dos Óculos. Todos os direitos reservados.</p>
            <Link href="/admin" className="flex items-center gap-1 text-blue-100/40 hover:text-[#7FB2FF] transition-colors" title="Área Administrativa">
          ...
          <div className="flex items-center gap-3">
            {['PIX', 'Visa', 'MC', 'Boleto'].map((method) => (
              <div key={method} className="h-7 px-3 bg-white/5 border border-white/10 rounded-md flex items-center justify-center text-blue-100/60 text-xs font-bold">
                {method}
              </div>
            ))}
          </div>
